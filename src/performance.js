...
export function createPerformance({scene,camera,controls,actor,stage,ring,setTheme,onEnter,onExit,toast}) {
 ...
 function loadAudio(url,name,useLrc){
  ...
  audio.onloadedmetadata=()=>{if(!Number.isFinite(audio.duration)||audio.duration<=0){removeAudio();toast('無法讀取音檔長度');return;}audioReady=true;duration=audio.duration;$('#show-scrub').max=duration;$('#audio-status').textContent=name+' · '+format(duration)+(synced?' · LRC 逐句同步':' · 等分字幕時間');$('.show-note').textContent=synced?'原曲 MP3 × LRC 同步演出 · 20.60 秒前奏 · 33 句字幕': '自訂音檔 · 字幕依曲長平均分配，未套用原曲 LRC';$('#remove-audio').hidden=false;seek(0);};
